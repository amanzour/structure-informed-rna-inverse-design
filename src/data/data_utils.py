...
from src.data.sec_struct_utils import pdb_to_sec_struct, pdb_to_sec_struct_bp
...
def pdb_to_tensor_2(
    ...
    sec_bp = None
    # fr3d_bp = None
    pdb_map = dict(zip(df.residue_id.unique(),np.arange(1,len(sequence)+1)))
    if return_sec_struct:
        # get secondary structure
        sec_bp = pdb_to_sec_struct_bp(filepath, sequence, pdb_map, keep_pseudoknots)
    ...
    return sequence, coords, sec_struct, sasa, sec_bp

def get_twist(x, i, j, unpaired_pos, unpaired_idx):
    my_twist = 0.0

    d = dist_2(x[i], x[j])
    pos = abs(unpaired_idx[unpaired_pos[i]] - unpaired_idx[unpaired_pos[j]])
    if d > 2:
        my_twist = pos/d
        my_twist = my_twist.item()
    else:
        my_twist = 0.0
    return my_twist

def dist_2(V1: torch.Tensor, V2: torch.Tensor) -> torch.Tensor:
