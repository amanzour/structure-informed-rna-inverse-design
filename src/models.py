...
class AutoregressiveMultiGNNv1(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        h_V_all = (batch.node_s, batch.node_v)
         
        edge_index_list = batch.edge_index_list
        p_s_t_map_list = batch.p_s_t_map_list
        seq = batch.seq

        h_V_all = self.W_v(h_V_all)  # (n_nodes, n_conf, d_s), (n_nodes, n_conf, d_v, 3)
        
        h_V_final = []
        for k in range(len(edge_index_list)):
            h_V = (h_V_all[0][:,k,:], h_V_all[1][:,k,:,:])
            edge_index = edge_index_list[k].to(torch.int64)
            p_s_t_map = p_s_t_map_list[k]
            h_E = (batch.edge_s_list[k], batch.edge_v_list[k])
            h_E = self.W_e(h_E)  # (n_edges, d_se), (n_edges, d_ve, 3)

            for layer in self.encoder_layers:
                h_V = layer(h_V, edge_index, h_E, p_s_t_map)  # (n_nodes, n_conf, d_s), (n_nodes, n_conf, d_v, 3)
            ...
            # ==================================
            h_V = (h_V[0].unsqueeze(dim=1),h_V[1].unsqueeze(dim=1))
            h_V_final.append(h_V)
            # ==================================
        
        # ======================================
        h_V_final = list(zip(*h_V_final))
        h_V = (torch.cat(h_V_final[0], dim = 1), torch.cat(h_V_final[1], dim = 1))    
        h_V = self.pool_multi_conf_2(h_V, batch.mask_confs)
        # ======================================
    ...
    def pool_multi_conf_2(self, h_V, mask_confs):

        if mask_confs.size(1) == 1:
            # Number of conformations is 1, no need to pool
            return (h_V[0][:, 0], h_V[1][:, 0])
