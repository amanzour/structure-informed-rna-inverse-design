...
class MultiGVPConvLayer(nn.Module):
    ...
    def __init__(
        ...
        # =======================modification=================
        # replacing self.conv with the other three movels makes it edge-type aware
        # self.conv = MultiGVPConv(node_dims, node_dims, edge_dims, n_message,
        #                         aggr="mean", activations=activations, vector_gate=vector_gate)
        self.conv_p = MultiGVPConv(node_dims, node_dims, edge_dims, n_message,
                                 aggr="mean", activations=activations, vector_gate=vector_gate)
        self.conv_s = MultiGVPConv(node_dims, node_dims, edge_dims, n_message,
                                 aggr="mean", activations=activations, vector_gate=vector_gate)
        self.conv_t = MultiGVPConv(node_dims, node_dims, edge_dims, n_message,
                                 aggr="mean", activations=activations, vector_gate=vector_gate)
    ...
    def forward(self, x, edge_index, edge_attr, p_s_t_map):
        '''
        :param x: tuple (s, V) of `torch.Tensor`
        :param edge_index: array of shape [2, n_edges]
        :param edge_attr: tuple (s, V) of `torch.Tensor`
        '''
        # Modifying for edge-type awareness replacing dh with the other three dh outputs
        norm_weights = torch.nn.functional.softmax(self.weights, dim=0)
        if self.norm_first:
            dh_p = self.conv_p(self.norm[0](x), edge_index[:,p_s_t_map == 0], 
                               (edge_attr[0][p_s_t_map == 0,:], edge_attr[1][p_s_t_map == 0,:]))
            dh_s = self.conv_s(self.norm[0](x), edge_index[:,p_s_t_map == 1], 
                               (edge_attr[0][p_s_t_map == 1,:], edge_attr[1][p_s_t_map == 1,:]))
            dh_t = self.conv_t(self.norm[0](x), edge_index[:,p_s_t_map == 2], 
                              (edge_attr[0][p_s_t_map == 2,:], edge_attr[1][p_s_t_map == 2,:]))
            
            dh_p = (norm_weights[0] * dh_p[0], norm_weights[0] * dh_p[1])
            dh_s = (norm_weights[1] * dh_s[0], norm_weights[1] * dh_s[1])
            dh_t = (norm_weights[2] * dh_t[0], norm_weights[2] * dh_t[1])
            dh = tuple_sum(dh_p, dh_s, dh_t)
...
class MultiGVPConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_v = x_v.contiguous().view(x_v.shape[0], x_v.shape[1] * 3)
        # =====end=============================
    ...
    def message(self, s_i, v_i, s_j, v_j, edge_attr):
        ...
        v_i = v_i.view(v_i.shape[0], self.vi, 3)
        v_j = v_j.view(v_j.shape[0], self.vi, 3)
        # ==========end========================
...
def _split_multi(x, ns, nv, n_conf=5):
    ...
    s = x[..., :-3 * nv].contiguous().view(x.shape[0], ns)
    v = x[..., -3 * nv:].contiguous().view(x.shape[0], nv, 3)
    # ==================end=============================
